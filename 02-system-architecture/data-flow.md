...
| Anomaly index | Single value | Float (project-defined normalized index — not a probability) | 4 bytes |
| Anomaly record | Structured record | JSON / DB record | ~200–500 bytes |

## Data Flow Rates

`Assumption`: Based on a candidate 50 Hz sampling rate (chosen to provide margin above the 40 Hz Nyquist minimum). A higher sampling rate such as 100 Hz may be selected to provide additional practical margin for anti-alias filtering.
